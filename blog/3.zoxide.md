...
> zoxide 是一个可以快速跳转到常用文件地址的工具 使用起来十分方便，只需要 `z note`就可以从任意目录跳转到常用的note文件夹，但是需要先访问一次
...
安装完成后，需要将 `zoxide` 集成到你使用的命令行环境 在 PowerShell 中输入 `code $PROFILE` 打开配置文件 并添加以下内容后保存
